...
- **On-site assistant:** a **dependency-free, self-hosted** keyword/synonym chat widget - **no third-party AI, no outbound API calls** - with an admin-editable knowledge base, branch data pulled from the CMS, an unanswered-question analytics log, and a lead-capture handoff to WhatsApp / phone.
- **Content automation:** an **auto-template router** applies the right design to a page by slug/title (so the Q&A hub and every sub-topic get the article layout with zero manual setup), and the on-page accordions **feed the `FAQPage` schema** automatically.
